...
void ft_enemy_events(t_game_instance *game_init) 
{
	...
	{ // move para a direita
		if (enemy_col == game_init->map_init.cols_matrice - 2 || game_init->map_init.matrice[enemy_row][enemy_col + 1] != EMPTY) 
		{
			enemy_direction = -1;
			enemy_col--;
		} else
	...
	{ // move para a esquerda
		if (enemy_col == 1 || game_init->map_init.matrice[enemy_row][enemy_col - 1] != EMPTY) 
		{
			enemy_direction = 1;
			enemy_col++;
		} else
